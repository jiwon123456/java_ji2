...
public class StuAction {
	...
	int stuInput(int stuCount) {
		while(true) {
			...
			System.out.printf("%d 번쨰 학생 이름을 입력하세요.(1.이전페이지 이동)\n",stuCount);
			...
			System.out.printf("%d 번쨰 학생성적이 저장되었습니다.\n",stuCount);
			System.out.println();
			stuCount++;
	...
	void stuUpdate() {
		...
		while(true) {
			...
			System.out.printf("찾고자 하는 학생 이름을 입력하세요.(1.이전페이지 이동)\n");
			String name = scan.next();
			if (name.equals("0")) { // 이전페이지이동
				System.out.println(">> 이전페이지를 이동합니다!!");
				System.out.println();
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (name.equals(s.getName())) {
					...
					case 1:
						...
						s.setTotal(s.getKor() + s.getEng() + s.getMath()); // 점수를 읽은 후 바로 합계 수정 저장
					...
					case 3:
						...
						s.setTotal(s.getKor() + s.getEng() + s.getMath()); // 점수를 읽은 후 바로 합계 수정 저장
						s.setAvg(s.getTotal() / 3.0); // 평균 수정
						System.out.println("수학" + "점수가 변경되었습니다.");
						System.out.println();
						break;
	...
	void stuDelete() {
		while(true) {
			...
			System.out.printf("삭제하고자 하는 학생 이름을 입력하세요.(1.이전페이지 이동)\n");
			String name = scan.next();
			if (name.equals("0")) { // 이전페이지이동
				System.out.println(">> 이전페이지를 이동합니다!!");
				System.out.println();
